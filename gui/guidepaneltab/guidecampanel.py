...
AUTOSAVE_FORMAT = '/%a_%H:%M:%S:%f.fits'
...
class GuideCamPanel(wx.Panel):
    def __init__(self, parent):
        ...
        ################################################################
        self.autosave_directory = datetime.datetime.now().strftime(AUTOSAVE_DIR)
    ...
    def takePicture(self):
        ...
        if self.autosave_cb.GetValue():
            fname = datetime.datetime.now().strftime(
                self.autosave_directory + AUTOSAVE_FORMAT)
            try:
                if not os.path.isdir(self.autosave_directory):
                    os.makedirs(self.autosave_directory)
                    logging.debug("Created autosave directory "
                          + self.autosave_directory)
                with open(fname, 'wb') as image:
                    i.writeto(image)
                    image.close()
                    logging.debug('Image autosaved to ' + fname)
            except IOError as e:
                wx.MessageBox(
                    'Warning, image could not be autosaved to %s!' % fname,
        ...
        if self.take_series.GetValue():
            ...
            try:
                ...
                wx.CallAfter(wx.MessageBox,
                             'Please select cadence time in seconds!',
    ...
    def setExposure(self):
        ...
        try:
            self.parent.cam.setExposure(
                int(1000 * float(self.exposure.GetValue())))
        except:
            wx.CallAfter(wx.MessageBox,
                         'Please select exposure time in secconds!',
                         'INVALID EXPOSURE TIME!', wx.OK | wx.ICON_ERROR)
            raise
